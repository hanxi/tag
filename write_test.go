...
func TestWriteTag_M4A_ReturnsUnsupportedForNow(t *testing.T) {
	tmp, err := os.CreateTemp(t.TempDir(), "stub-*.m4a")
	if err != nil {
		t.Fatalf("CreateTemp: %v", err)
	}
	tmp.Close()
	...
	err = WriteTag(tmp.Name(), WriteOptions{Title: "x"})
	if !errors.Is(err, ErrUnsupportedWrite) {
		t.Errorf("M4A should currently return ErrUnsupportedWrite, got %v", err)
	}
}
